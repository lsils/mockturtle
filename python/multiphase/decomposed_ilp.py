...
TYPES = {"0": "PI", "1": "PO", "2": "AA", "3": "AS", "4": "SA"}

class Primitive:
    __slots__ = ['sig', 'func', 'type', 'fanins', 'phase']
    def __init__(self, _sig: int, _func : int, _type : str, _fanins : list, _phase : int = None) -> None:
        self.sig = _sig
        self.func = _func
        self.type = _type
        self.fanins = _fanins
...
if __name__ == "__main__":
    ...
    # NPH = 4
    S_bounds = ( 0, int(1000/NPH) )      #Clock stage bounds
    ...
    S : dict[int, model.IntVar] = {}
    Phi : dict[int, model.IntVar] = {}
    fanin_constr = {}
    ctr = 0
    for g in all_signals.values():
        Phi[g.sig] = model.NewIntVar(*phi_bounds, f'phi_{g.sig}')
        hint_S, hint_phi = divmod(g.phase, NPH) if g.phase is not None else (None, None)
        
        if hint_phi:
            model._CpModel__model.solution_hint.vars.append(ctr)
            model._CpModel__model.solution_hint.values.append(hint_phi)
            ctr += 1
        ...
        if g.type == 'PI':
            ...
            # if hint_S:
            model._CpModel__model.solution_hint.vars.append(ctr)
            model._CpModel__model.solution_hint.values.append(0)
            ctr += 1
        else:
            S[g.sig] = model.NewIntVar(*S_bounds, f'S_{g.sig}')  
            # if hint_S:
            model._CpModel__model.solution_hint.vars.append(ctr)
            model._CpModel__model.solution_hint.values.append(hint_S)  
            ctr += 1
    ...
    if (solver.StatusName(status) in ("OPTIMAL", "FEASIBLE")):
        ...
        for s, phi in zip(S.values(), Phi.values()):
            ...
            print(f'{signal}:{solver.Value(s), solver.Value(phi)}, {solver.Value(s)*NPH+solver.Value(phi)}')
