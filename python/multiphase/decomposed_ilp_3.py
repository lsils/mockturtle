...
class Primitive:
    ...
    def __repr__(self):
        return f"Primitive(sig={self.sig}, func={self.func}, type='{self.type}', fanins={self.fanins}, phase={self.phase})"
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            if line.startswith('PI'):
                ...
                _sig_str, _func_str, _type_str, _fanin_str, *_gphase_str = list_split
                _sig  = int(_sig_str)
                _func = int(_func_str)
                _gphase = int(_gphase_str[0]) # if _gphase_str else None
                _fanins = [int(_fanin_sig) for _fanin_sig in _fanin_str.split('|') if _fanin_sig]
                
                # *** Need to add fanout information - the phase 
