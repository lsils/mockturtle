...
namespace mockturtle {
    ...
    vector<node<Ntk>> get_pis( const Ntk& ntk ) {
        vector<node < Ntk>> pis;
     ...
     */
    template <typename Ntk>
    vector<signal<Ntk>> get_pos( const Ntk& ntk ) {
        vector<signal < Ntk>> pos;
     ...
     */
    template <typename Ntk>
    vector<signal<Ntk>> get_children( const Ntk& ntk, node<Ntk> parent ) {
    ...
    namespace detail {
        ...
        signal<Ntk> replace_in_subgraph_rec( Ntk& ntk, signal<Ntk> root_sig, signal<Ntk> old_sig, signal<Ntk> new_sig, T& cache ) {
            if ( root_sig == old_sig ) return new_sig;
            if ( root_sig == !old_sig ) return !new_sig;
    ...
    struct distributivity {
        ...
        distributivity( node<Ntk> n, signal<Ntk> x, signal<Ntk> u, signal<Ntk> y, signal<Ntk> v, signal<Ntk> z, direction dir ) :
        n( n ), x( x ), u( u ), y( y ), v( v ), z( z ), dir( dir ) {
        }
    ...
    namespace detail {
        ...
        template <typename Ntk>
        void get_bwd_distributivities( const Ntk& ntk, node<Ntk> n, signal<Ntk> v_sig, signal<Ntk> xuy_sig, signal<Ntk> xuz_sig, vector<distributivity<Ntk>>&result ) {
     ...
     */
    template <typename Ntk>
    vector<distributivity<Ntk>> get_bwd_distributivities( Ntk& ntk, node<Ntk> n ) {
        vector<distributivity < Ntk>> res;
        if ( ntk.is_maj( n ) ) {
    ...
    namespace detail {
        ...
        template <typename Ntk>
        void get_fwd_distributivities( const Ntk& ntk, node<Ntk> n, signal<Ntk> x_sig, signal<Ntk> u_sig, signal<Ntk> yvz_sig, vector<distributivity<Ntk>>&result ) {
     ...
     */
    template <typename Ntk>
    vector<distributivity<Ntk>> get_fwd_distributivities( Ntk& ntk, node<Ntk> n ) {
        vector<distributivity < Ntk>> res;
        if ( ntk.is_maj( n ) ) {
            vector<signal < Ntk>> c = get_children( ntk, n );
            // for all cyclic permutations
            for ( auto i = 0; i < 3; i++ ) {
    ...
    namespace detail {
        ...
        template <typename Ntk>
        void get_associativities( const Ntk& ntk, node<Ntk> n, signal<Ntk> x_sig, signal<Ntk> u_sig, signal<Ntk> yuz_sig, vector<associativity<Ntk>>&result ) {
     ...
     */
    template <typename Ntk>
    vector<associativity<Ntk>> get_associativities( const Ntk& ntk, const node<Ntk> n ) {
        vector<associativity < Ntk>> res;
        if ( ntk.is_maj( n ) ) {
            auto c = get_children( ntk, n );
    ...
    struct relevance {
        ...
        relevance( node<Ntk> n, signal<Ntk> x, signal<Ntk> y, signal<Ntk> z ) :
        n( n ), x( x ), y( y ), z( z ) {
        }
    ...
    template <typename Ntk>
    vector<relevance<Ntk>> get_fwd_comp_assocs( Ntk& ntk, node<Ntk> n ) {
        vector<relevance < Ntk>> result;
        if ( ntk.is_maj( n ) ) {
            auto c = get_children( ntk, n );
            uint64_t ind[3] = {0, 1, 2};
            // for all permutations
    ...
    template <typename Ntk>
    vector<relevance<Ntk>> get_bwd_comp_assocs( Ntk& ntk, node<Ntk> n ) {
        vector<relevance < Ntk>> result;
        if ( ntk.is_maj( n ) ) {
            auto c = get_children( ntk, n );
            uint64_t ind[3] = {0, 1, 2};
     ...
     */
    template <typename Ntk>
    vector<relevance<Ntk>> get_relevances( Ntk& ntk, node<Ntk> n ) {
        vector<relevance < Ntk>> result;
        if ( ntk.is_maj( n ) ) {
            auto c = get_children( ntk, n );
            uint64_t ind[3] = {0, 1, 2};
    ...
    namespace detail {
        ...
        template <typename Ntk>
        void get_swappings( const Ntk& ntk, node<Ntk> n, signal<Ntk> x_sig, signal<Ntk> a_sig, signal<Ntk> b_sig, vector<swapping<Ntk>>&result, const node_map<kitty::dynamic_truth_table, Ntk>& tts ) {
     ...
     */
    template <typename Ntk>
    vector<swapping<Ntk>> get_swappings( const Ntk& ntk, const node<Ntk> n ) {
    ...
    bool is_u_and_v_similar_wrt_candidate( const Ntk& ntk, signal<Ntk> u_sig, signal<Ntk> v_sig, node<Ntk> candidate ) {
        ...
        if ( !ntk.is_maj( u ) || !ntk.is_maj( v ) ) return false;
        ...
        if ( stat[0][0] && stat[1][1] && stat[2][2] ) return true;
        if ( stat[0][0] && stat[1][2] && stat[2][1] ) return true;
        if ( stat[0][1] && stat[1][0] && stat[2][2] ) return true;
        if ( stat[0][1] && stat[1][2] && stat[2][0] ) return true;
        if ( stat[0][2] && stat[1][0] && stat[2][1] ) return true;
        if ( stat[0][2] && stat[1][1] && stat[2][0] ) return true;
        return false;
    }
    
    template<typename Ntk>
    vector<node<Ntk>> get_replacee_candidate_nodes( Ntk& ntk, signal<Ntk> u, signal<Ntk> v ) {
        vector<node < Ntk>> result;
    ...
    template<typename Ntk>
    vector<node<Ntk>> get_replacee_candidate_pis( Ntk& ntk, signal<Ntk> u, signal<Ntk> v ) {
        default_simulator<kitty::dynamic_truth_table> sim( ntk.num_pis() );
        const auto tts = simulate_nodes<kitty::dynamic_truth_table>( ntk, sim );
    ...
    template<typename Ntk>
    void find_suitable_replacer( Ntk& ntk, vector<node<Ntk>>&replacee_list, signal<Ntk> root, map<pair<node<Ntk>, uint64_t>, uint64_t>& ht, unordered_map<node<Ntk>, bool>& visited ) {
        node<Ntk> root_node = ntk.get_node( root );
        
        if ( !ntk.is_maj( root_node ) || visited[root_node] ) return;
        ...
        for ( auto& replacee : replacee_list ) {
            for ( auto i = 0; i < 3; i++ ) {
                if ( ntk.get_node( c[i] ) == replacee ) {
                    for ( auto j = 0; j < 3; j++ ) {
                        if ( j != i ) {
                            pair<node<Ntk>, uint64_t> temp = make_pair( replacee, c[j].data );
                            ht[temp]++;
                        }
                    }
                }
    ...
    template<typename Ntk>
    vector<pair<node<Ntk>, signal<Ntk>>> find_suitable_replacee_replacer_pairs( Ntk& ntk, vector<node<Ntk>>&replacee_list, signal<Ntk>& root_r ) {
        map<pair<node<Ntk>, uint64_t>, uint64_t> ht;
        unordered_map < node<Ntk>, bool> visited;
        
        find_suitable_replacer( ntk, replacee_list, root_r, ht, visited );
        
        unsigned long max = 0;
        vector<pair<node<Ntk>, signal < Ntk>>> result;
     ...
     */
    template<typename Ntk>
    vector<symmetry<Ntk>> get_symmetries( Ntk& ntk, node<Ntk> n ) {
        vector<symmetry < Ntk>> result;
        if ( ntk.is_maj( n ) ) {
            auto c = get_children( ntk, n );
            for ( auto i = 0; i < 3; i++ ) {
                signal<Ntk> x = c[i];
    ...
    struct majn_substitution {
        node<Ntk> n;
        vector<signal<Ntk>> signals;
        
        majn_substitution( node<Ntk> n, vector<signal<Ntk>> signals ) : n( n ), signals( signals ) {
        }
    };
     ...
     */
    template<typename Ntk>
    vector<signal<Ntk>> maj_of( kitty::dynamic_truth_table& tt ) {
        vector<signal < Ntk>> result;
        vector<signal < Ntk>> empty;
    ...
    template<typename Ntk>
    signal<Ntk> relabel_recursively( Ntk& ntk, Ntk& opt_ntk, signal<Ntk> opt_root, vector<signal<Ntk>>&sigs ) {
        signal<Ntk> result;
        ...
        if ( opt_ntk.is_constant( opt_root_node ) ) {
            ...
            vector< signal < Ntk>> c = get_children( opt_ntk, opt_root_node );
            result = ntk.create_maj(
     ...
     */
    template<typename Ntk>
    signal<Ntk> replace_with_opt_maj_n( Ntk& ntk, node<Ntk> node, vector<signal<Ntk>> sigs ) {
        Ntk opt_ntk;
        if ( sigs.size() == 5u ) {
            std::array<signal<Ntk>, 5> pis;
            for ( uint64_t i = 0; i < 5; i++ ) pis[i] = opt_ntk.create_pi();
            opt_ntk.create_po( majority5( opt_ntk, pis ) );
        } else if ( sigs.size() == 7u ) {
            std::array<signal<Ntk>, 7> pis;
            for ( uint64_t i = 0; i < 7; i++ ) pis[i] = opt_ntk.create_pi();
            opt_ntk.create_po( majority7( opt_ntk, pis ) );
        } else if ( sigs.size() == 9u ) {
            std::array<signal<Ntk>, 9> pis;
            for ( uint64_t i = 0; i < 9; i++ ) pis[i] = opt_ntk.create_pi();
     ...
     */
    template<typename Ntk>
    signal<Ntk> replace_with_opt_maj_n( Ntk& ntk, signal<Ntk> root, map<node<Ntk>, vector<signal<Ntk>>>&subs ) {
        node<Ntk> root_node = ntk.get_node( root );
        signal<Ntk> result;
        if ( ntk.is_constant( root_node ) ) {
            ...
            if ( subs.count( root_node ) > 0 ) {
                result = replace_with_opt_maj_n( ntk, root_node, subs[root_node] );
            } else {
                vector<signal < Ntk>> c = get_children( ntk, root_node );
                result = ntk.create_maj(
    ...
    signal<Ntk> substitute_maj_n( Ntk& ntk, node<Ntk> root_node, uint64_t num_vars ) {
        ...
        vector<majn_substitution < Ntk>> subs;
        default_simulator<kitty::dynamic_truth_table> sim( ntk.num_pis() );
        ...
        ntk.foreach_node( [&]( auto n ) {
            if ( ntk.is_maj( n ) ) {
                vector<signal < Ntk>> result = maj_of<Ntk>( ttmap[n] );
        ...
        if ( subs.size() > 1 ) {
            ...
            for ( auto i = 0u; i < num_vars; i++ ) {
                ...
                if ( !found ) {
                    for ( auto j = i; j > 0; j-- ) {
                        swap( subs[0].signals[j], subs[0].signals[j - 1] );
                    }
                    break;
                }
            }
            ...
            for ( auto i = 0u; i < num_vars; i++ ) {
                ...
                if ( !found ) {
                    for ( auto j = i; j > 0; j-- ) {
                        swap( subs[1].signals[j], subs[1].signals[j - 1] );
                    }
                    break;
                }
            }
        ...
        map<node<Ntk>, vector < signal < Ntk>>> subs_map;
