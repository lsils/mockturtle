...
void simulate_modular_arithmetic( uint32_t k, uint64_t c, ArithFn&& operation, EvaluateFn&& evaluate, Ops... ops )
{
  ...
  for ( auto i = 0u; i < NumOps; ++i )
  {
    ...
    std::generate( signals[i].begin(), signals[i].end(), [&ntk]()
                   { return ntk.create_pi(); } );
  }

  std::apply( [&]( auto&&... args )
              { operation( ntk, args..., c ); },
              signals );

  std::for_each( signals[0].begin(), signals[0].end(), [&]( auto f )
                 { ntk.create_po( f ); } );

  CHECK( ntk.num_pis() == NumOps * k );
  CHECK( ntk.num_pos() == k );

  const auto assign = std::accumulate( aops.rbegin(), aops.rend(), 0u, [&]( auto a, auto v )
                                       { return ( a << k ) + v; } );
  const auto simm = simulate<bool>( ntk, input_word_simulator( assign ) );
  CHECK( simm.size() == k );
  const auto result = std::apply( [&]( auto&&... args )
                                  { return evaluate( args..., c ); },
                                  aops );
...
{
  test_binary_modular_arithmetic<aig_network>( []( auto& ntk, auto& a, auto const& b, uint64_t c )
                                               { modular_adder_inplace( ntk, a, b, c ); },
                                               []( auto a, auto b, auto c )
                                               { return ( a + b ) % c; } );
  test_binary_modular_arithmetic<mig_network>( []( auto& ntk, auto& a, auto const& b, uint64_t c )
                                               { modular_adder_inplace( ntk, a, b, c ); },
                                               []( auto a, auto b, auto c )
                                               { return ( a + b ) % c; } );
  test_binary_modular_arithmetic<xag_network>( []( auto& ntk, auto& a, auto const& b, uint64_t c )
                                               { modular_adder_inplace( ntk, a, b, c ); },
                                               []( auto a, auto b, auto c )
                                               { return ( a + b ) % c; } );
}
...
{
  test_binary_modular_arithmetic<aig_network>( []( auto& ntk, auto& a, auto const& b, uint64_t c )
                                               { modular_adder_hiasat_inplace( ntk, a, b, c ); },
                                               []( auto a, auto b, auto c )
                                               { return ( a + b ) % c; } );
  test_binary_modular_arithmetic<xag_network>( []( auto& ntk, auto& a, auto const& b, uint64_t c )
                                               { modular_adder_hiasat_inplace( ntk, a, b, c ); },
...
{
  test_unary_modular_arithmetic<aig_network>( []( auto& ntk, auto& a, uint64_t c )
                                              { modular_doubling_inplace( ntk, a, c ); },
                                              []( auto a, auto c )
                                              { return ( a * 2 ) % c; } );
  test_unary_modular_arithmetic<mig_network>( []( auto& ntk, auto& a, uint64_t c )
                                              { modular_doubling_inplace( ntk, a, c ); },
                                              []( auto a, auto c )
                                              { return ( a * 2 ) % c; } );
  test_unary_modular_arithmetic<xag_network>( []( auto& ntk, auto& a, uint64_t c )
                                              { modular_doubling_inplace( ntk, a, c ); },
                                              []( auto a, auto c )
                                              { return ( a * 2 ) % c; } );
}
...
TEST_CASE( "build default modular halving", "[modular_arithmetic]" )
{
  ...
  for ( auto i = 0; i < 1000; ++i )
  {
    ...
    simulate_modular_arithmetic<aig_network>(
        k, c, []( auto& ntk, auto& a, uint64_t c )
        { modular_halving_inplace( ntk, a, c ); },
        []( auto a, auto c )
        { return a % 2 ? ( a + c ) / 2 : a / 2; },
        a );
...
TEST_CASE( "build Montgomery multiplier", "[modular_arithmetic]" )
{
  ...
  std::for_each( pos.begin(), pos.end(), [&]( auto const& f )
                 { xag.create_po( f ); } );
...
TEST_CASE( "10-bit constant multiplication by 661", "[modular_arithmetic]" )
{
  ...
  std::for_each( sum.begin(), sum.end(), [&]( auto const& f )
                 { xag.create_po( f ); } );
