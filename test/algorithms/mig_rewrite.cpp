...
TEST_CASE( "distributivity (forward direction)", "[mig_rewrite]" ) {
    ...
    vector<distributivity < mig_network>> dists = get_fwd_distributivities( mig, 7u );
    CHECK( dists.size() == 3u );
    for ( auto& dist : dists ) {
        CHECK( dist.n == 7u );
...
TEST_CASE( "distributivity (backward direction)", "[mig_rewrite]" ) {
    ...
    vector<distributivity < mig_network>> dists = get_bwd_distributivities( mig, 8u );
...
TEST_CASE( "associativity", "[mig_rewrite]" ) {
    ...
    vector<associativity < mig_network>> assocs = get_associativities( mig, 6u );
...
mig_network create_relevance_test_network() {
    ...
    mig.create_po( mig.create_maj( pis[0], pis[1], mig.create_maj(
                                                                  pis[2], mig.create_maj( pis[0], pis[1], !pis[2] ), mig.create_maj( pis[0], !pis[1], pis[2] ) ) ) );
    return mig;
}

TEST_CASE( "relevance", "[mig_rewrite]" ) {
    auto mig = create_relevance_test_network();
    vector<relevance < mig_network>> relevs = get_relevances( mig, 7u );
...
TEST_CASE( "complement associativity forward", "[mig_rewrite]" ) {
    ...
    vector<relevance < mig_network>> cassocs = get_fwd_comp_assocs( mig, 5u );
...
TEST_CASE( "complement associativity backward", "[mig_rewrite]" ) {
    auto mig = create_comp_assoc_test_network();
    vector<relevance < mig_network>> cassocs = get_bwd_comp_assocs( mig, 5u );
    CHECK( cassocs.size() == 1u );
    for ( auto& cassoc : cassocs ) {
        auto temp = create_comp_assoc_test_network();
        temp.substitute_node( 5u, cassoc.apply_to( temp ) );
...
mig_network create_swapping_test_network() {
    ...
    auto [mig, pis] = init_network<mig_network, 5>();
    mig.create_po( mig.create_maj( pis[0], mig.create_maj( pis[1], pis[2], pis[3] ), mig.create_maj(
                                                                                                    pis[1], mig.create_and( pis[2], pis[3] ), mig.create_and( pis[3], pis[4] ) ) ) );
...
TEST_CASE( "swapping", "[mig_rewrite]" ) {
    ...
    vector<swapping < mig_network>> swappings = get_swappings( mig, 10u );
...
TEST_CASE( "symmetry", "[mig_rewrite]" ) {
    ...
    vector<symmetry < mig_network>> symmetries = get_symmetries( mig, 22u );
...
vector<node<Ntk>> get_nodes( const Ntk& ntk ) {
    vector<node < Ntk>> nodes;
...
bool apply_rule_backward( Ntk& ntk, FnRules&& get_rules ) {
    ...
    for ( auto it = nodes.rbegin(); it != nodes.rend(); it++ ) {
        if ( apply_first( ntk, *it, get_rules ) ) return true;
    }
    return false;
}
...
/**
 * String representation of a signal.
 */
string to_string( mig_network::signal sig ) {
    return (sig.complement ? "~" : "" ) +to_string( sig.index );
}

/**
 * String representation of a mig rooted at a given node.
 */
string to_string( const mig_network& mig, mig_network::node n ) {
    if ( mig.is_maj( n ) && !mig.is_pi( n ) ) {
        std::array<mig_network::signal, 3> c;
        mig.foreach_fanin( n, [&c]( auto const& f, auto i ) {
            c[i] = f;
        } );
        return string( "(" ) + to_string( n ) + string( ")" ) + string( "<" )
        + ( ( c[0].complement > 0 ) ? "~" : "" ) + to_string( mig, mig.get_node( c[0] ) ) + ( mig.is_dead( mig.get_node( c[0] ) ) ? "d" : "" ) + ","
        + ( ( c[1].complement > 0 ) ? "~" : "" ) + to_string( mig, mig.get_node( c[1] ) ) + ( mig.is_dead( mig.get_node( c[1] ) ) ? "d" : "" ) + ","
        + ( ( c[2].complement > 0 ) ? "~" : "" ) + to_string( mig, mig.get_node( c[2] ) ) + ( mig.is_dead( mig.get_node( c[2] ) ) ? "d" : "" ) + ">";
    } else {
        return to_string( n );
    }
}
...
/**
 * Print a given mig recursively.
 */
void print_mig_1( const mig_network& mig ) {
    mig_network::signal root = get_pos( mig )[0];
    cout << "\n--- begin graph ---\n";
    cout << "pis = " << mig.num_pis() << ", size = " << mig.num_gates() << endl;
    cout << ( root.complement > 0 ? "~" : "" ) << to_string( mig, mig.get_node( root ) ) << endl;
    cout << "---  end graph  ---\n\n";
}
