...
TEST_CASE( "node and signal iteration in an MIG", "[mig]" )
{
  ...
  mask = counter = 0;
  mig.foreach_po( [&]( auto s, auto i )
                  { mask |= ( 1 << mig.get_node( s ) ); counter += i; } );
  CHECK( mask == 48 );
  CHECK( counter == 1 );

  mask = 0;
  mig.foreach_po( [&]( auto s )
                  { mask |= ( 1 << mig.get_node( s ) ); } );
  CHECK( mask == 48 );

  mask = counter = 0;
  mig.foreach_po( [&]( auto s, auto i )
                  { mask |= ( 1 << mig.get_node( s ) ); counter += i; return false; } );
  CHECK( mask == 16 );
  CHECK( counter == 0 );

  mask = 0;
  mig.foreach_po( [&]( auto s )
                  { mask |= ( 1 << mig.get_node( s ) ); return false; } );
  CHECK( mask == 16 );

  /* iterate over gates */
  mask = counter = 0;
  mig.foreach_gate( [&]( auto n, auto i )
                    { mask |= ( 1 << n ); counter += i; } );
  CHECK( mask == 48 );
  CHECK( counter == 1 );
  ...
  /* iterate over fanins */
  mask = counter = 0;
  mig.foreach_fanin( mig.get_node( f1 ), [&]( auto s, auto i )
                     { mask |= ( 1 << mig.get_node( s ) ); counter += i; } );
  CHECK( mask == 14 );
  CHECK( counter == 3 );

  mask = 0;
  mig.foreach_fanin( mig.get_node( f1 ), [&]( auto s )
                     { mask |= ( 1 << mig.get_node( s ) ); } );
  CHECK( mask == 14 );
