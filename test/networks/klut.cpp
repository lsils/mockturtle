...
TEST_CASE( "node and signal iteration in a k-LUT network", "[klut]" )
{
  ...
  klut.foreach_node( [&]( auto n, auto i )
                     { mask |= ( 1 << n ); counter += i; } );
  CHECK( mask == 63 );
  CHECK( counter == 15 );
  ...
  /* iterate over PIs */
  mask = counter = 0;
  klut.foreach_pi( [&]( auto n, auto i )
                   { mask |= ( 1 << n ); counter += i; } );
  CHECK( mask == 12 );
  CHECK( counter == 1 );

  mask = 0;
  klut.foreach_pi( [&]( auto n )
                   { mask |= ( 1 << n ); } );
  CHECK( mask == 12 );
  ...
  klut.foreach_po( [&]( auto s, auto i )
                   { mask |= ( 1 << klut.get_node( s ) ); counter += i; } );
  CHECK( mask == 48 );
  CHECK( counter == 1 );

  mask = 0;
  klut.foreach_po( [&]( auto s )
                   { mask |= ( 1 << klut.get_node( s ) ); } );
  CHECK( mask == 48 );

  mask = counter = 0;
  klut.foreach_po( [&]( auto s, auto i )
                   { mask |= ( 1 << klut.get_node( s ) ); counter += i; return false; } );
  CHECK( mask == 16 );
  CHECK( counter == 0 );

  mask = 0;
  klut.foreach_po( [&]( auto s )
                   { mask |= ( 1 << klut.get_node( s ) ); return false; } );
  CHECK( mask == 16 );
}
