...
void test_mapping_view()
{
  ...
  CHECK( !has_is_mapped_v<Ntk> );
  CHECK( !has_clear_mapping_v<Ntk> );
  CHECK( !has_num_luts_v<Ntk> );
  CHECK( !has_add_to_mapping_v<Ntk> );
  CHECK( !has_remove_from_mapping_v<Ntk> );
  CHECK( !has_lut_function_v<Ntk> );
  CHECK( !has_set_lut_function_v<Ntk> );
  CHECK( !has_foreach_lut_fanin_v<Ntk> );

  using mapped_ntk = mapping_view<Ntk>;

  CHECK( has_has_mapping_v<mapped_ntk> );
  CHECK( has_is_mapped_v<mapped_ntk> );
  CHECK( has_clear_mapping_v<mapped_ntk> );
  CHECK( has_num_luts_v<mapped_ntk> );
  CHECK( has_add_to_mapping_v<mapped_ntk> );
  CHECK( has_remove_from_mapping_v<mapped_ntk> );
  CHECK( !has_lut_function_v<mapped_ntk> );
  CHECK( !has_set_lut_function_v<mapped_ntk> );
  CHECK( has_foreach_lut_fanin_v<mapped_ntk> );

  using mapped_ntk_f = mapping_view<Ntk, true>;

  CHECK( has_has_mapping_v<mapped_ntk_f> );
  CHECK( has_is_mapped_v<mapped_ntk_f> );
  CHECK( has_clear_mapping_v<mapped_ntk_f> );
  CHECK( has_num_luts_v<mapped_ntk_f> );
  CHECK( has_add_to_mapping_v<mapped_ntk_f> );
  CHECK( has_remove_from_mapping_v<mapped_ntk_f> );
  CHECK( has_lut_function_v<mapped_ntk_f> );
  CHECK( has_set_lut_function_v<mapped_ntk_f> );
  CHECK( has_foreach_lut_fanin_v<mapped_ntk_f> );
};
